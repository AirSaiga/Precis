...
- [安装](#安装)
  ...
  - [快速安装（源码）](#快速安装源码)
  - [手动安装](#手动安装)
  - [首次运行注意事项（下载安装包用户）](#首次运行注意事项下载安装包用户)
- [启动方式](#启动方式)
...
无需任何前置条件，直接从 [GitHub Releases](https://github.com/AirSaiga/Precis/releases) 下载对应平台的安装包：
...
下载后双击安装即可。首次运行因缺少代码签名，系统会弹出安全警告，请参阅下方[首次运行注意事项](#首次运行注意事项)。

> 💡 Precis 当前处于 Alpha 阶段，尚未购买代码签名证书。这是正常现象，不代表软件有问题。正式版会补充签名。
...
由于 Precis Alpha 版本未经代码签名，首次运行时操作系统会弹出安全警告。这是正常现象，请按以下步骤处理：

#### Windows（SmartScreen 警告）

1. 双击安装程序 `.exe`
2. 看到"Windows 已保护你的电脑"蓝色窗口 → 点击 **"更多信息"**
3. 点击 **"仍要运行"**
4. 按提示完成安装

> 如果安装后首次启动 Precis 时再次弹出警告，重复上述步骤即可。
> 若想彻底消除警告：右键 `.exe` → 属性 → 勾选底部的 **"解除锁定"** → 确定 → 再运行。

#### macOS（Gatekeeper 拦截）

**情况一：打开 DMG 时提示"无法验证开发者"或"已损坏"**

打开"终端"（Terminal），执行（注意替换为你实际的路径和版本号）：
```bash
...
右键点击 Precis.app → 选择 **"打开"** → 在弹窗中点击 **"打开"**。仅需操作一次，之后双击即可正常启动。

> 上述操作仅移除 macOS 的隔离标记（quarantine），不会影响系统安全。正式签名版本发布后这些步骤将不再需要。
