...
| TUI（终端 UI） | Rust + ratatui + crossterm + tokio + reqwest（`tui-rust/`，通过 HTTP 调 Python 后端，独立于 Electron/前端） |
...
**约束三层命名映射**（新增约束类型时三处必须一致）：ConstraintKind / ConstraintNodeType / V2Type 的映射**单一事实源**是 `services/constraints/constraintMeta.ts` 的 `CONSTRAINT_TYPES`，参考副本与新增约束注册清单见 ARCHITECTURE.md。
...
| **ValidationRegistry** | `services/constraints/validationRegistryCore.ts`（barrel，已拆分为 `constraintMeta.ts`/`handlerRegistry.ts`/`validationExecutors.ts` 等子模块） | 执行约束校验 |
...
- **含非 JSON 类型（Date/Map/Set/RegExp 等）的对象**：必须用 `structuredClone()`。`JSON.parse(JSON.stringify(...))` 会静默丢失这些类型（Date→string、RegExp→空对象、Map/Set→空）。
- **纯 JSON 配置数据（manifest / 数据源 / 快捷键配置等 YAML round-trip 数据）**：`JSON.parse(JSON.stringify(...))` 可接受——数据本身就是 JSON 安全的，且对小对象性能更好、可读性更高。
- **Vue reactive proxy**：不可直接 `structuredClone`（会抛 "could not be cloned"）。注意 `toRaw()` 只解包顶层 proxy，嵌套属性仍是 proxy，深拷贝嵌套 reactive 对象时需递归解包或改用 JSON 方式。
- **不确定数据是否含非 JSON 类型时**：优先用 `structuredClone()`（安全兜底）。
...
3. **约束三层命名映射** — ConstraintKind / ConstraintNodeType / V2Type 必须一致（**单一事实源**：`constraintMeta.ts` 的 `CONSTRAINT_TYPES`，参考副本见 ARCHITECTURE.md）
4. **前端类型** — `frontend/src/types/nodes.ts` 添加对应的 `*NodeData` 接口
5. **约束类型名 i18n** — 在 `frontend/src/i18n/locales/{zh-CN,en-US}/constraints.ts` 的 `constraintTypes.<kind>.{name,description}` 补双侧条目（key 为 camelCase 的 ConstraintKind）。菜单/节点库/布局器统一从此命名空间取显示名，不再在各自组件里硬编码。注意：`ConstraintNodeRegistration` 接口已不含 `displayName`/`description`。
...
新增/修改 `t('key')` 引用或语言包 key 时，必须保证 zh-CN 与 en-US **双侧都有定义**，否则 `audit:i18n` 守卫失败（已接入 `lint` / `lint:check` / 前端 CI）。
