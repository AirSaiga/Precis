...
排查、测试与审计中发现缺陷时按两级红线分流，防止"全部顺手修"与"全部入 backlog"两种漂移：

| 级别 | 判据 | 处理方式 |
|------|------|---------|
| **立即修** | ① 崩溃/未捕获 traceback；② 数据误判（校验误报或漏报）；③ 契约破坏（CLI JSON 输出结构、退出码语义、V2 YAML 格式、API 状态码） | 当前批次内修复并附带可复现测试，不跨批滞留 |
| **可延迟** | 非数量级性能问题、文案瑕疵、罕见平台边缘行为、GUI 非阻断问题 | 记入 backlog（CHANGELOG 遗留清单或 GitHub issue），不阻断当前批次 |

数据误判是校验工具的生命线：误报让用户关掉规则，漏报让用户失去信任，两者都比崩溃更伤产品。GitHub issue 模板的"影响类型"字段与此分级对应。
