...
**废弃文件**（保留供参考，新代码应使用策略）：
- `utils/nodes/schema/columnGeneration.ts` — 迁移至 TabularColumnGenerator
- `utils/nodes/json/columnGeneration.ts` — 迁移至 JsonColumnGenerator

**使用示例**：
```typescript
// 键盘处理器中
generateSchema.ts → 使用 previewDataFetcher + columnGenerator
bindDataSource.ts → 使用 previewDataFetcher + columnGenerator

// Composable 中
useNodeSourceManager → options.generateColumns 传入策略实例
```
