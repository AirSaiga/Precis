...
- **行为约束**：这是 refactor，不是改行为。三个函数的输入→输出映射应保持等价——包括边界行为
  （`getDataAsString` 在 `data === null` 时抛 `TypeError` 也是 seed 的既有行为，必须保持；
  verify 会剥掉类型注解动态执行代码、逐条对照）。
- **类型守卫要求**：至少引入 2 处真正的类型守卫（`typeof x === ...`、`'key' in obj`、或自定义
  `isXxx` 谓词函数）。仅把 `as unknown as` 换成单层 `as` 而不引入守卫，不算合格重构
  （守卫计数会剔除注释后再比对 seed 基线，注释里写守卫关键词不算数）。

### 约束（务必遵守）

- **`as unknown as` 双重断言与 `as any` 都被禁止；其它单层 `as X` 是允许的**（`unknown as X` 永远合法）。
- 只改 `workspace/code.ts`。
- 不碰 `seed/`、`verify.mjs`、`task.md`、`SOLUTION.md`。
- 不碰 `workspace/` 以外的任何文件（即不要改主仓库代码）。
- 不引入新的 `import`（保持文件自包含）。
- 不用 `as any`、`as any as` 之类的变体绕过。
...
退出码 0 = PASS，非 0 = FAIL。verify 结合**静态检查**与**动态行为验证**（不跑 tsc；动态执行前会剥掉
TS 类型注解）：`as unknown as` 归零、`as any` 变体归零、类型守卫净增 ≥2（剔注释后总数 ≥3 = seed
基线 1 + 净增 2）、3 个导出函数仍存在、未引入新 import、以及三个函数的行为与 seed 逐条对照等价
（含 `data === null` 时 `getDataAsString` 仍抛 `TypeError`）。详见 verify 输出。
