...
解析失败透出：get_project_overview 把解析失败的配置文件记入 parse_errors
（文件路径 + 错误摘要，含 YAML 语法错误与缺必填字段的严格校验失败），
...
class ReadProjectTool:
    ...
    def _truncate_overview(overview: dict[str, Any]) -> dict[str, Any]:
        ...
        parse_errors（解析失败清单）同样保留前 N 项——它是诊断入口，不能静默裁掉。
        ...
        result: dict[str, Any] = {
            ...
            "parse_errors": parse_errors[:_MAX_PARSE_ERRORS_IN_OBSERVATION],
        }
        ...
        result["truncated_parse_error_count"] = max(0, len(parse_errors) - _MAX_PARSE_ERRORS_IN_OBSERVATION)
        return result
