...
用类型系统编码动作 spec 的**结构性**约束（枚举、必填等），
取代散落各处的手写 dict.get() + if 校验。

设计原则：
- 只做**结构校验**（不依赖项目运行时状态）：枚举值、字段存在性。
- **上下文校验**（表/列/FK 是否存在、类型兼容性、参数完整性）仍留在各 validator 函数——
  它们需要运行时加载的 schema，Pydantic 表达不了。
- 与 registry.py 互补：registry 描述"有哪些动作"，specs 描述"每个动作的 spec 长什么样"。
- 约束参数的完整性校验（如 Range min/max、Scripted expression/pattern）由
  _constraint_validator 与 constraint_builder 负责，不在 Pydantic 层做。
...
class ConstraintSpec(BaseModel):
    ...
    # Scripted 空表达式等参数关系校验由 constraint_builder 落实。
