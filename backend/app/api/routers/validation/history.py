...
安全约定:
- 项目路径统一从 `X-Project-Config-Path` Header 获取（经 get_project_config_path 校验），
  不再从请求体/Query 接受 project_path，避免任意目录读写。
"""
...
def _get_store(project_path: str) -> ValidationHistoryStore:
    ...
    参数:
        project_path: 项目配置根目录（已由 get_project_config_path 校验为存在的绝对路径）
...
class SaveRunRequest(BaseModel):
    """保存校验运行记录请求（项目路径从 Header 注入，不在 body 中）"""
...
def save_run(req: SaveRunRequest, project_path: str = Depends(get_project_config_path)) -> dict[str, Any]:
    """
    ...
    参数:
        ...
        project_path: 项目配置根目录（Header 注入）
...
def list_runs(
    project_path: str = Depends(get_project_config_path),
