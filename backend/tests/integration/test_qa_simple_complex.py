"""
@fileoverview qa_simple 复杂项目集成测试

用 qa_test/qa_simple 统一测试项目作为 fixture，端到端验证
schema → 约束 → 正则 → 校验完整链路：
...
qa_simple 是统一测试集（12 个 Schema + 18 个 Regex + 5 个 Template +
5 个 Template Instance + 多个内嵌约束 + 引用完整性问题），用于覆盖：
- ID 编码（XOR + Base64URL）解析
- 模板展开（age_check + user_quality_check + required_field + numeric_range + normalize_enum）
- 内嵌约束 + 独立约束
- 数据文件加载（CSV + JSON + JSONL）
- 加载阶段警告与错误的传播
"""
...
@pytest.fixture
def qa_simple_project(tmp_path: Path) -> Path:
    """提供 qa_simple 项目的副本 fixture。"""
    return _copy_qa_simple_into(tmp_path)
...
class TestQaSimpleManifestLoading:
    """验证 qa_simple manifest 引用全部解析正确。"""

    def test_load_qa_simple_returns_loaded_project(self, qa_simple_project: Path):
        """load_project 应返回 LoadedProject 实例，不抛异常。"""
        result = load_project(str(qa_simple_project / "project.precis.yaml"))
        assert result is not None
        assert result.manifest is not None
        # 清单中声明了 12 个 schema 引用，至少应解析出对应的 schema_files
        assert len(result.schema_files) >= 10

    def test_load_qa_simple_manifest_lists_all_schemas(self, qa_simple_project: Path):
        """manifest.schemas 应至少包含 10 条 Schema 引用。"""
        result = load_project(str(qa_simple_project / "project.precis.yaml"))
        assert len(result.manifest.schemas) >= 10

    def test_load_qa_simple_resolves_all_regex_refs(self, qa_simple_project: Path):
        """18 个 regex_nodes 引用应全部被解析为 RegexNodeFile。"""
        result = load_project(str(qa_simple_project / "project.precis.yaml"))
        # manifest 中声明了 18 个 regex（1 简单 + 17 复杂）
        assert len(result.manifest.regex_nodes) == 18
        # 应全部加载成功
        assert len(result.regex_node_files) == 18

    def test_load_qa_simple_resolves_template_refs(self, qa_simple_project: Path):
        """5 个 template 引用应全部被解析。"""
        result = load_project(str(qa_simple_project / "project.precis.yaml"))
        assert len(result.manifest.templates) == 5
        # 验证模板 ID 列表
        ...
        assert "required_field" in template_ids
        assert "numeric_range" in template_ids
        assert "normalize_enum" in template_ids

    def test_load_qa_simple_resolves_template_instances(self, qa_simple_project: Path):
        """5 个 template_instances 应在 manifest 中存在。"""
        result = load_project(str(qa_simple_project / "project.precis.yaml"))
        assert len(result.manifest.template_instances) == 5
...
class TestQaSimpleTemplateExpansion:
    """验证 qa_simple 中的模板实例正确展开。"""

    def test_template_age_check_expands_to_range_constraint(self, qa_simple_project: Path):
    ...
    def test_template_user_quality_check_expands_multiple_nodes(self, qa_simple_project: Path):
    ...
    def test_template_expansion_uses_namespaced_ids(self, qa_simple_project: Path):
...
class TestQaSimpleConstraintAggregation:
    """验证内嵌约束与模板展开约束合并到 constraint_files。"""

    def test_embedded_constraints_are_collected(self, qa_simple_project: Path):
        ...
        # qa_simple 在 customers/users/employees 等 schema 中有 13+ 条内嵌约束
    ...
    def test_combined_constraint_count_exceeds_manifest_count(self, qa_simple_project: Path):
...
class TestQaSimpleValidationExecution:
    """对 qa_simple 执行校验，验证不崩溃、返回结果结构。"""

    def test_execute_qa_simple_returns_result(self, qa_simple_project: Path):
        """对完整项目执行校验，应返回结果（不抛异常）。"""
        manifest = str(qa_simple_project / "project.precis.yaml")
        data_dir = str(qa_simple_project / "data")
    ...
    def test_execute_qa_simple_runs_without_unhandled_exception(self, qa_simple_project: Path):
    ...
    def test_execute_qa_simple_detects_data_violations(self, qa_simple_project: Path):
    ...
    def test_execute_with_table_filter_subset(self, qa_simple_project: Path):
...
class TestQaSimpleLoadingDiagnostics:
    """验证 qa_simple 已知的引用完整性问题被收集为警告/错误。"""

    def test_load_collects_warnings_or_errors_for_integrity_issues(self, qa_simple_project: Path):
        """qa_simple 含 ID 不匹配、外键目标表缺失等已知问题，应至少产生警告或错误。"""
        result = load_project(str(qa_simple_project / "project.precis.yaml"))
        # 已知会有 IDMismatchWarning + ReferenceIntegrityError
        total_signals = len(result.warnings or []) + len(result.loading_errors or [])
        assert total_signals > 0, "qa_simple 已知存在引用完整性问题，应至少有 1 条信号"

    def test_loading_errors_have_diagnostic_metadata(self, qa_simple_project: Path):
        """LoadingError 应带有 error_type、message、suggestion 等诊断字段。"""
        result = load_project(str(qa_simple_project / "project.precis.yaml"))
        if not result.loading_errors:
            pytest.skip("qa_simple 未产生 loading_errors，跳过诊断字段检查")
...
class TestQaSimpleApiEndpoints:
    """通过 FastAPI TestClient 端到端验证 V2 API。"""

    def test_get_manifest_via_api(self, client, qa_simple_project: Path):
        """GET /api/latest/project/manifest 应返回 200 + schemas/constraints/regex_nodes 列表。"""
        resp = client.get(
            "/api/latest/project/manifest",
            headers={"X-Project-Config-Path": str(qa_simple_project)},
        )
        ...
        assert len(manifest["regex_nodes"]) == 18

    def test_list_templates_via_api(self, client, qa_simple_project: Path):
        """GET /api/latest/project/template 应返回 5 个模板。"""
        resp = client.get(
    ...
    def test_get_template_by_id_via_api(self, client, qa_simple_project: Path):
    ...
    def test_preview_template_expand_age_check_via_api(self, client, qa_simple_project: Path):
    ...
    def test_preview_template_expand_user_quality_check_via_api(self, client, qa_simple_project: Path):
    ...
    def test_get_schema_by_id_via_api(self, client, qa_simple_project: Path):
    ...
    def test_get_regex_by_id_via_api(self, client, qa_simple_project: Path):
    ...
    def test_nonexistent_template_returns_404(self, client, qa_simple_project: Path):
...
class TestQaSimpleNodeConsistency:
    """验证前端视角下的节点数量与磁盘 YAML 文件数量一致。"""

    def test_loaded_schema_count_matches_yaml_files(self, qa_simple_project: Path):
        """加载出的 schema_files 数量应等于 schemas/ 目录下 .schema.yaml 文件数量。"""
        schemas_dir = qa_simple_project / "schemas"
        schema_files_on_disk = list(schemas_dir.glob("*.schema.yaml"))
        result = load_project(str(qa_simple_project / "project.precis.yaml"))
        # 加载层可能将 manifest 引用解析为文件。允许加载数量 ≤ 磁盘文件数。
        assert len(result.schema_files) <= len(schema_files_on_disk)
        assert len(result.schema_files) >= len(schema_files_on_disk) - 2

    def test_loaded_regex_count_matches_yaml_files(self, qa_simple_project: Path):
        """加载出的 regex_node_files 数量应等于 regex_nodes/ 目录下 .regex.yaml 文件数量。"""
        regex_dir = qa_simple_project / "regex_nodes"
        regex_files_on_disk = list(regex_dir.glob("*.regex.yaml"))
        result = load_project(str(qa_simple_project / "project.precis.yaml"))
        assert len(result.regex_node_files) == len(regex_files_on_disk)

    def test_loaded_template_count_matches_yaml_files(self, qa_simple_project: Path):
        """加载出的 template_files 数量应等于 templates/ 目录下 .template.yaml 文件数量。"""
        templates_dir = qa_simple_project / "templates"
