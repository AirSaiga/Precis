...
class TestRootEndpoint:
    ...
    def test_openapi_schema_generated(self, client):
        ...
        # 关键路由应存在
        assert "/project/v2/manifest" in spec["paths"]
        assert "/connection-rules" in spec["paths"]
...
class TestProjectConfigPathHeader:
    ...
    def test_missing_header_returns_422(self, client):
        resp = client.get("/project/v2/manifest")
    ...
    def test_nonexistent_directory_returns_404(self, client):
        ...
        resp = client.get("/project/v2/manifest", headers={"X-Project-Config-Path": fake_path})
...
class TestManifestRoute:
    ...
    def test_get_manifest_returns_project_content(self, client, project_dir, project_header):
        resp = client.get("/project/v2/manifest", headers=project_header)
        assert resp.status_code == 200
        body = resp.json()
    ...
    def test_put_manifest_merges_by_default(self, client, project_dir, project_header):
        ...
        resp = client.put("/project/v2/manifest", json=new_manifest, headers=project_header)
        assert resp.status_code == 200

        # 重新读取，应包含原有的 users 和新加的 extra（合并模式）
        resp2 = client.get("/project/v2/manifest", headers=project_header)
        body = resp2.json()
...
class TestViewRoute:
    ...
    def test_get_view_returns_default_when_missing(self, client, project_dir, project_header):
        resp = client.get("/project/v2/view", headers=project_header)
        assert resp.status_code == 200
        body = resp.json()
    ...
    def test_put_then_get_view_roundtrip(self, client, project_dir, project_header):
        ...
        resp = client.put("/project/v2/view", json=payload, headers=project_header)
        assert resp.status_code == 200
        assert resp.json()["message"]

        # 重新读
        resp2 = client.get("/project/v2/view", headers=project_header)
        body = resp2.json()
    ...
    def test_view_file_persisted_as_json(self, client, project_dir, project_header):
        ...
        client.put("/project/v2/view", json=payload, headers=project_header)
...
class TestWorkspacesRoute:
    ...
    def test_get_workspaces_returns_default(self, client, project_dir, project_header):
        resp = client.get("/project/v2/workspaces", headers=project_header)
        assert resp.status_code == 200
        body = resp.json()
        assert body["version"] == 1
    ...
    def test_put_then_get_workspaces(self, client, project_dir, project_header):
        ...
        resp = client.put("/project/v2/workspaces", json=payload, headers=project_header)
        assert resp.status_code == 200

        resp2 = client.get("/project/v2/workspaces", headers=project_header)
        body = resp2.json()
...
class TestConnectionRulesRoute:
    ...
    def test_get_returns_empty_when_file_missing(self, client, project_dir, project_header):
        resp = client.get("/connection-rules", headers=project_header)
        assert resp.status_code == 200
        body = resp.json()
    ...
    def test_put_then_get_roundtrip(self, client, project_dir, project_header):
        ...
        resp = client.put("/connection-rules", json=rules, headers=project_header)
        assert resp.status_code == 200

        resp2 = client.get("/connection-rules", headers=project_header)
        body = resp2.json()
    ...
    def test_reset_removes_file(self, client, project_dir, project_header):
        ...
        client.put("/connection-rules", json=rules, headers=project_header)
        rules_file = os.path.join(project_dir, "connection-rules.precis.yaml")
        assert os.path.isfile(rules_file)

        # 重置
        resp = client.post("/connection-rules/reset", headers=project_header)
        assert resp.status_code == 200
...
class TestWorkspaceDataSourcesRoute:
    ...
    def test_get_default_workspace(self, client, project_dir, project_header):
        resp = client.get("/workspace/config", headers=project_header)
        assert resp.status_code == 200
        body = resp.json()
    ...
    def test_add_data_source_persists(self, client, project_dir, project_header):
        ...
        resp = client.post("/workspace/data-sources", json=ds, headers=project_header)
        assert resp.status_code == 200
        body = resp.json()
        ...
        # 再次 GET 验证持久化
        resp2 = client.get("/workspace/config", headers=project_header)
    ...
    def test_add_duplicate_data_source_updates_existing(self, client, project_dir, project_header):
        ...
        client.post("/workspace/data-sources", json=ds, headers=project_header)
    ...
    def test_remove_data_source(self, client, project_dir, project_header):
        ...
        resp = client.delete("/workspace/data-sources/ds-1", headers=project_header)
        assert resp.status_code == 200
    ...
    def test_clear_all_data_sources(self, client, project_dir, project_header):
        ...
        resp = client.delete("/workspace/data-sources", headers=project_header)
        assert resp.status_code == 200
