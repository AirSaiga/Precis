...
export interface ConstraintValidationResult {
  ...
  /**
   * 错误消息（key 化形式，i18n 治理）。
   * 渲染端应优先用 localizedErrors 经 renderLocalizedMessage 解析（随 locale 切换）；
   * 未提供时回退 validationErrors 字符串。error_code → validation.codes.<CODE>，
   * 行级错误带 row（渲染为 validation.rowError 行前缀）。
   */
