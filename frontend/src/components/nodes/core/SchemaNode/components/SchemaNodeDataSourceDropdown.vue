/** * @file SchemaNodeDataSourceDropdown.vue * @description Schema节点数据源下拉选择组件 * *
核心功能： * - 以树状结构显示外部数据源栏的数据 * - 支持选择数据源并自动创建 SourcePreview 节点 * -
支持切换数据源时断开旧连接、智能填充列定义 * * 数据流： * 外部数据源栏 → 树状结构 → 下拉菜单 →
Schema节点数据源绑定/切换 */
<template>
  <Teleport to="body">
      ...
      >
        ...
        <div class="dropdown-header">
          <span class="header-title">{{
            t('customNodes.schemaNode.source.currentConnection')
          }}</span>
        </div>
        ...
        <div class="dropdown-header">
          <span class="header-title">{{
            t('customNodes.schemaNode.source.availableSources')
          }}</span>
        </div>
...
<script setup lang="ts">
  /**
   * @file SchemaNodeDataSourceDropdown.vue
   * @description Schema节点数据源下拉菜单组件
   *
   * 该组件负责以树状结构显示数据源选择下拉菜单，支持：
   * 1. 显示当前已连接的数据源
   * 2. 以树状结构列出外部数据源栏的数据源
   * 3. 处理数据源切换逻辑
   */
  ...
  const shouldShowFile = (fileItem: DataSourceTreeItem): boolean => {
    if (!fileItem.folderPath) {
      // 没有 folderPath 的文件直接显示
      return true
    }

    // 检查文件所在的所有父文件夹是否都是展开状态
  ...
  const isCurrentSource = (dataSource: ExternalDataSource): boolean => {
    ...
    return props.currentSource.sourceName === dataSource.name
  }
...
<style scoped src="./SchemaNodeDataSourceDropdown.styles.css"></style>
