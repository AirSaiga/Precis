...
export interface UseConstraintLayoutOptions {
  ...
  getLastValidation?: () => { errorCount?: number; totalRows?: number; matchCount?: number } | undefined
  /** 是否启用自动校验 */
...
export function buildMetrics(
  ...
  return [
    { label: t('customNodes.constraintRules.totalRows', '总行数'), value: lastValidation.totalRows || 0 },
    { label: t('customNodes.constraintRules.matchCount', '匹配'), value: lastValidation.matchCount || 0 },
    { label: t('customNodes.constraintRules.errorCount', '错误'), value: lastValidation.errorCount || 0 }
  ]
}
