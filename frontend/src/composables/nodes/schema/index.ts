...
type SchemaNodeEmits = {
  ...
  'add-column': []
  'pattern-bind': [string, Record<string, unknown>]
  'constraint-create': [Record<string, unknown>]
  'remove-node': [string]
  'schema-node-save': [{ nodeId: string; nodeData: SchemaNodeData }]
}
...
export function useSchemaNode(
  ...
  emit: EmitFn<SchemaNodeEmits>
) {
  // Schema数据管理
  // 处理 Schema 节点的数据初始化、更新、同步等核心功能
  const data = useSchemaData(props, emit as unknown as EmitFn<{ dataChanged: [SchemaNodeData] }>)

  // Schema数据源连接管理
  // 处理 Schema 节点与数据源节点的连接状态管理
  const sourceManager = useSchemaSourceManager(
    props,
  ...
  const editing = useSchemaEditing(props, emit as (event: string, ...args: unknown[]) => void)
  ...
  const events = useSchemaEvents(props, emit as (event: string, ...args: unknown[]) => void)
