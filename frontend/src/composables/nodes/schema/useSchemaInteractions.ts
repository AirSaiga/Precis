...
export function useSchemaInteractions(
   ...
   * @param event - 连接事件对象
   */
  const handleColumnOutputConnect = (event: { handleId: string; targetNodeId: string; targetHandleId: string }) => {
   ...
   */
  const createTableRelation = (sourceColumnId: string, targetColumnId: string, targetNodeId: string) => {
