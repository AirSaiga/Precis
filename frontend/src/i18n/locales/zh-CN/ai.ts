...
const aiChat = {
  ...
  // 画布对账（v2 变更集：磁盘重读重建画布后的消息级同步摘要）
  canvasSyncDone: '画布已同步：新增 {added}、更新 {updated}、移除 {removed}',
  canvasSyncPartial: '画布部分同步：新增 {added}、更新 {updated}、移除 {removed}，{failed} 项失败',
  canvasSyncFailedTitle: '画布同步失败',
  canvasSyncFailedMessage: '配置已写盘，画布同步失败，建议重新加载项目',
  // apply 确认 / ask 回答回传失败（卡片保持可重试）
  confirmSendFailed: '确认请求发送失败，请重试',
  respondSendFailed: '回答发送失败，请重试',
  // 工具轨迹折叠卡
