...
const messages = {
  ...
  common: {
    ...
    folderProcessFailed: '处理文件夹 {name} 失败',
    fileImportFailed: '处理文件 {name} 失败：{error}',
    addDataSourceFailed: '添加数据源 {name} 失败：{error}',
    fileUpdated: '文件已更新：{name}，您现在可以将此数据源拖拽到画布中使用。',
    reselectFailed: '重新选择文件失败，请稍后重试',
    openFileFailed: '打开文件失败：{error}，请尝试重新选择文件。',
    confirmReselectFile:
      '无法打开文件：{name}\n\n错误信息：{error}\n\n可能的原因：\n• 文件已被移动或删除\n• 路径已变更\n• 没有关联的程序打开此文件类型\n\n是否重新选择文件？',
    folderNoDataFiles: '文件夹 "{name}" 中没有找到数据文件',
    scanFolderFailed: '扫描文件夹失败: {name}',
    folderImportSummary: '文件夹导入完成：成功 {success} 个，失败 {fail} 个',
    folderImportFailed: '文件夹导入失败: {error}',
    electronFileSelectFailed: '文件选择失败，请稍后重试',
  ...
  persistence: {
    ...
    projectSavedWithDrafts:
      '项目 "{name}" 已保存（{count} 个未完成的草稿节点未保存，配置数据源或连线后自动纳入）',
