...
export function createTemplateExpandModule(params: {
  ...
  async function expandOnCanvasInner(instanceNodeId: string, expandResult: TemplateExpandResult) {
    ...
    const { dagNodes, dagEdges } = buildDagPlan(items, nodes.value)
  ...
  // --------------------------------------------------------------------------
  // Stage 1/2: 解析 API 结果 + 构建 DAG 规划
  // （纯函数，实现在 templateExpandPlanning.ts）
  // --------------------------------------------------------------------------
