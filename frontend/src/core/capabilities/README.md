...
| ---------------- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ | --------------------------------------------------------------------- |
| `appApi.ts`      | 应用级能力：版本、后端端口/状态、最近项目持久化、后端重启、界面语言同步、后端 API token 获取 | IPC (`getAppVersion` / `getServerStatus` / `loadConfig` / `saveConfig` / `restartPythonServer` / `setAppLocale` / `getApiToken`) | HTTP API + `localStorage`                        | `canRestoreRecentProject` / `canSyncAppLocale`                        |
...
| `feedbackApi.ts` | 崩溃反馈：崩溃日志持久化、反馈导出、pending crash 读取/清除、日志尾部读取                    | IPC（主进程写 `userData/feedback/`）                                                                                             | 浏览器下载 / 空操作                              | 无（通过方法行为判断）                                                |
