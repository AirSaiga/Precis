  ...
  <div class="action-bar">
    <div class="action-bar-main">
      ...
      <button
        ...
        :title="!store.activeProvider?.is_configured ? t('aiConfigGenerator.errors.modelNotConfigured') : ''"
        @click="emit('generate')"
      >
        <span v-if="generating" class="spinner-sm"></span>
        {{ generating ? t('aiConfigGenerator.actions.generating') : t('aiConfigGenerator.actions.startGeneration') }}
      </button>
      ...
      <button
        ...
        @click="emit('cancel')"
      >
        {{ canceling ? t('aiConfigGenerator.actions.canceling') : t('aiConfigGenerator.actions.cancel') }}
      </button>
    </div>
    ...
    <div v-if="fileListExpanded && selectedPaths.length" class="file-list-dropdown">
      <div class="file-list-header">
        ...
        <div class="header-actions">
          <button class="text-btn" @click="emit('toggle-all')" :disabled="isExpanding">
            {{ checkedFiles.size === expandedFiles.length ? t('aiConfigGenerator.actions.selectNone') : t('aiConfigGenerator.actions.selectAll') }}
          </button>
          <button class="clear-btn" @click="emit('clear'); fileListExpanded = false">
      ...
      <div class="file-list-scroll">
        ...
        >
          <input type="checkbox" :checked="checkedFiles.has(p)" @click.stop="emit('toggle-file', p)" />
