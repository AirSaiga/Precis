...
| 后端 | Python 3.10+ · FastAPI + Uvicorn + Pydantic + Pandas |
...
> **Electron 生产打包说明**：当前安装包仅打包后端源码，运行时需目标机器已安装 Python `>=3.10,<3.14` 并在安装目录的 `resources/backend` 下执行 `pip install -e ".[api]"`。详见 [`electron/README.md`](electron/README.md)。
...
| Backend | Python 3.10+ · FastAPI + Uvicorn + Pydantic + Pandas |
...
> **Electron production packaging note**: The installer currently ships the backend source only. The target machine must have Python `>=3.10,<3.14` installed, and dependencies must be installed in `resources/backend` via `pip install -e ".[api]"`. See [`electron/README.md`](electron/README.md) for details.
